...
import { Stethoscope, Phone, Lock, AlertCircle, ChevronUp } from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <main ref={mainRef} className="snap-y snap-mandatory overflow-y-scroll h-screen scroll-smooth hide-scrollbar">
      {/* القسم الأول: صفحة الاستقبال */}
      <section onClick={scrollToLogin} className="snap-start h-screen relative flex flex-col items-center justify-center text-center px-6 overflow-hidden bg-[#0a2540] cursor-pointer">
        <Image
          src="/images/hero-illustration.png"
          alt="المساعد الطبي الذكي"
          fill
          priority
        ...
        <button
          ...
          className="absolute bottom-10 z-10 flex flex-col items-center gap-2 text-white/80 hover:text-white transition-colors animate-bounce"
        >
          <ChevronUp className="w-5 h-5" />
        </button>
      </section>

      {/* القسم الثاني: تسجيل الدخول */}
      <section ref={loginRef} className="snap-start min-h-screen flex items-center justify-center bg-background-soft px-4">
